// Deterministic, dependency-free editorial "ink illustration" banners.
// No AI image generation is wired up (no image credits available) — instead every
// article/city/profession page gets a generated SVG banner in the site's own
// "Carta & Inchiostro" palette (accent #2F2AE5 ink on white paper), with a distinct
// line-art motif per content type and per-slug variation (gradient angle + scattered
// accent dots) so pages sharing a category don't look identical.
...
export const CATEGORY_MOTIF: Record<string, HeroMotif> = {
  'GUIDE PRATICHE': 'guide',
  'TECNOLOGIE & ATS': 'ats',
  'MERCATO & TREND': 'market',
  'COLLOQUIO': 'interview',
  'MODELLI & EUROPASS': 'europass',
};
...
export default function HeroArt({ motif, seed, width, height, className }: HeroArtProps) {
  const svg = buildHeroSvg({ motif, seed, width, height });
